...
<section className="py-12 sm:py-16 md:py-20 lg:py-24 bg-gradient-to-br from-slate-50 to-white">
  <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
    ...
    <div className="relative">
      {/* Navigation Arrows */}
      {/* <button 
        className="absolute left-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 -ml-4 md:-ml-6"
        onClick={() => document.getElementById('cards-container').scrollBy({ left: -300, behavior: 'smooth' })}
      >
        <ArrowRight className="w-5 h-5 text-blue-600 transform rotate-180" />
      </button>
      <button 
        className="absolute right-0 top-1/2 -translate-y-1/2 z-10 w-10 h-10 md:w-12 md:h-12 flex items-center justify-center bg-white rounded-full shadow-lg hover:shadow-xl transition-all duration-300 transform hover:scale-110 -mr-4 md:-mr-6"
        onClick={() => document.getElementById('cards-container').scrollBy({ left: 300, behavior: 'smooth' })}
      >
...
>
  <ArrowRight className="w-5 h-5 text-blue-600" />
      ...
      <div 
        id="cards-container"
        className="flex overflow-x-auto scroll-smooth snap-x snap-mandatory hide-scrollbar pb-8 px-2 sm:px-0"
      >
        {[
          {
            icon: <Building2 size={20} className="xs:w-6 xs:h-6 md:w-8 md:h-8" />,
            title: "Scalable Solutions",
            description: "We build systems that grow with your business, ensuring long-term success and adaptability to market changes.",
            gradient: "from-purple-500 to-pink-500",
          },
          {
            icon: <Shield size={20} className="xs:w-6 xs:h-6 md:w-8 md:h-8" />,
            title: "Security Approach",
            description: "We keep your data safe with strong, high-level security from the very beginning—so you never have to worry.",
            gradient: "from-slate-500 to-gray-600",
          },
          {
            icon: <Users size={20} className="xs:w-6 xs:h-6 md:w-8 md:h-8" />,
            title: "Expert Team",
            description: "Our team consists of seasoned professionals with deep expertise in cutting-edge technologies and methodologies.",
            gradient: "from-blue-500 to-cyan-500",
          },
          {
            icon: <Globe size={20} className="xs:w-6 xs:h-6 md:w-8 md:h-8" />,
            title: "Global Reach",
            description: "We serve clients worldwide, understanding diverse market needs and cultural nuances for better solutions.",
            gradient: "from-green-500 to-emerald-500",
          },
          {
            icon: <Sparkles size={20} className="xs:w-6 xs:h-6 md:w-8 md:h-8" />,
            title: "Custom Strategies",
            description: "We personally plan every project based on your specific needs and goals, ensuring the final result is a perfect fit for you.",
            gradient: "from-orange-500 to-red-500",
          },
        ].map((point, index) => (
          <div 
            key={index} 
            className="flex-shrink-0 w-[calc(100%-1rem)] sm:w-1/2 lg:w-1/3 px-2 snap-start animate-slide-up"
          >
            <div className="bg-white h-full p-6 sm:p-8 rounded-xl sm:rounded-2xl md:rounded-3xl shadow-lg md:shadow-xl border border-slate-100 hover:shadow-lg md:hover:shadow-xl transition-all duration-300 transform hover:scale-[1.02] md:hover:scale-105 card-hover">
              <div
                className={`w-10 h-10 xs:w-12 xs:h-12 md:w-16 md:h-16 bg-gradient-to-r ${point.gradient} rounded-lg sm:rounded-xl md:rounded-2xl flex items-center justify-center text-white mb-4 sm:mb-6 mx-auto`}
              >
                {point.icon}
              </div>
              <h3 className="text-base xs:text-lg md:text-xl font-bold mb-3 sm:mb-4 text-slate-900 text-center">{point.title}</h3>
              <p className="text-xs xs:text-sm md:text-base text-slate-600 leading-relaxed text-center">{point.description}</p>
            </div>
          </div>
        ))}
      </div>
...
<section className="py-12 sm:py-16 md:py-20 lg:py-24 bg-white overflow-hidden">
  <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
    ...
    <div className="relative w-full overflow-hidden py-4">
      <div className="flex items-center w-max animate-infinite-scroll-left hover:animation-paused space-x-4 sm:space-x-6 md:space-x-8">
        {[
          { name: "Next.js", color: "from-black to-gray-600" },
          { name: "React", color: "from-blue-500 to-cyan-500" },
          { name: "Tailwind", color: "from-cyan-500 to-blue-500" },
          ...
          <div
            key={index}
            className="flex-shrink-0 bg-white p-3 sm:p-4 md:p-6 rounded-lg sm:rounded-xl md:rounded-2xl border border-slate-200 hover:border-slate-300 hover:shadow-md md:hover:shadow-lg transition-all duration-300 transform hover:scale-[1.02] md:hover:scale-105 text-center min-w-[120px] sm:min-w-[150px]"
          >
            <div className={`w-6 h-6 xs:w-8 xs:h-8 md:w-12 md:h-12 bg-gradient-to-r ${tech.color} rounded-md sm:rounded-lg md:rounded-xl mx-auto mb-1 sm:mb-2 md:mb-3`}></div>
            <div className="font-semibold text-xs xs:text-sm md:text-base text-slate-900">{tech.name}</div>
          </div>
