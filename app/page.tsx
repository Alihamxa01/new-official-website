...
export default function Home() {
  return (
    <div className="pt-1">
      {/* Hero Section */}
      <section className="relative min-h-[60vh] sm:min-h-[65vh] md:min-h-[70vh] lg:min-h-[80vh] xl:min-h-screen flex items-center justify-center overflow-hidden floating-shapes">
        <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8 text-center relative z-10">
          <div className="animate-fade-in">
            <div className="inline-flex items-center px-2 py-1 sm:px-3 sm:py-1 md:px-4 md:py-2 bg-white/50 backdrop-blur-sm rounded-full border border-slate-200 mb-4 sm:mb-6 md:mb-8">
              <Sparkles className="w-3 h-3 sm:w-3 sm:h-3 md:w-4 md:h-4 text-blue-600 mr-1 sm:mr-1 md:mr-2" />
              <span className="text-xs sm:text-xs md:text-sm font-medium text-slate-700">Building Digital Intelligence</span>
            </div>
            ...
            <p className="text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl text-slate-600 mb-6 sm:mb-8 md:mb-12 max-w-2xl sm:max-w-3xl md:max-w-4xl mx-auto leading-relaxed px-2">
              We create powerful web & AI solutions that transform businesses and drive innovation in the digital age
            </p>
            <div className="flex flex-col xs:flex-row gap-2 xs:gap-3 sm:gap-4 justify-center px-2">
              <Link
                href="/contact"
                className="inline-flex items-center justify-center px-4 py-3 xs:px-6 xs:py-3 sm:px-8 sm:py-4 bg-gradient-to-r from-blue-600 to-blue-400 text-white font-semibold rounded-lg md:rounded-xl hover:shadow-xl transition-all duration-300 transform hover:scale-105 text-xs xs:text-sm sm:text-base"
              >
                {"Let's Work Together"}
                <ArrowRight className="ml-1 xs:ml-2 w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5" />
              </Link>
              <Link
                href="/services"
                className="inline-flex items-center justify-center px-4 py-3 xs:px-6 xs:py-3 sm:px-8 sm:py-4 bg-white border-2 border-slate-200 text-slate-700 font-semibold rounded-lg md:rounded-xl hover:border-slate-300 hover:shadow-lg transition-all duration-300 text-xs xs:text-sm sm:text-base"
              >
                Explore Services
                <Rocket className="ml-1 xs:ml-2 w-3 h-3 xs:w-4 xs:h-4 sm:w-5 sm:h-5" />
              </Link>
            </div>
          </div>
        </div>
        ...
        {/* Floating Elements */}
        <div className="absolute top-10 xs:top-16 sm:top-20 left-4 xs:left-6 sm:left-10 w-8 h-8 xs:w-10 xs:h-10 sm:w-12 sm:h-12 md:w-20 md:h-20 bg-gradient-to-r from-purple-400 to-pink-400 rounded-full opacity-20 animate-float"></div>
        <div
          className="absolute bottom-10 xs:bottom-16 sm:bottom-20 right-4 xs:right-6 sm:right-10 w-12 h-12 xs:w-16 xs:h-16 sm:w-20 sm:h-20 md:w-32 md:h-32 bg-gradient-to-r from-blue-400 to-cyan-400 rounded-full opacity-20 animate-float"
          style={{ animationDelay: "2s" }}
        ></div>
        <div
          className="absolute top-1/2 right-4 xs:right-6 sm:right-10 md:right-20 w-8 h-8 xs:w-10 xs:h-10 sm:w-12 sm:h-12 md:w-16 md:h-16 bg-gradient-to-r from-green-400 to-blue-400 rounded-full opacity-20 animate-float"
          style={{ animationDelay: "4s" }}
        ></div>
      </section>
